...
define Package/luci-app-ipv6prefixsnat/postrm
#!/bin/sh

[ -n "$${IPKG_INSTROOT}" ] && exit 0

rm -f /etc/config/ipv6prefixsnat
rm -rf /var/run/ipv6prefixsnat
rm -f /usr/share/nftables.d/ruleset-post/90-ipv6prefixsnat.nft
nft delete table ip6 ipv6prefixsnat_nat >/dev/null 2>&1 || true
/etc/init.d/rpcd reload >/dev/null 2>&1 || true
...
$(eval $(call BuildPackage,luci-app-ipv6prefixsnat))
